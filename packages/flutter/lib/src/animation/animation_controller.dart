...
class AnimationController extends Animation<double>
  ...
  TickerFuture _animateToInternal(
    ...
    final double scale = switch (animationBehavior) {
      ...
      .normal when SemanticsBinding.instance.disableAnimations => 0.05,
      .normal || .preserve => 1.0,
    };
    var simulationDuration = duration;
    if (simulationDuration == null) {
      assert(!(this.duration == null && _direction == .forward));
      assert(!(this.duration == null && _direction == .reverse && reverseDuration == null));
      final double range = upperBound - lowerBound;
      final double remainingFraction = range.isFinite ? (target - _value).abs() / range : 1.0;
      final Duration directionDuration = (_direction == .reverse && reverseDuration != null)
          ? reverseDuration!
          : this.duration!;
    ...
    if (simulationDuration == Duration.zero) {
      ...
      _status = (_direction == .forward) ? .completed : .dismissed;
      _checkStatusChanged();
  ...
  void _directionSetter(_AnimationDirection direction) {
    ...
    _status = (_direction == .forward) ? .forward : .reverse;
    _checkStatusChanged();
  }
  ...
  TickerFuture fling({
    ...
    final double scale = switch (behavior) {
      ...
      .normal when SemanticsBinding.instance.disableAnimations => 200.0,
      .normal || .preserve => 1.0,
    };
